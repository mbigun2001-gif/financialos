...
interface IncomeChartDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  incomeBySource: Array<{ name: string; value: number; percentage: number }>;
  totalIncome: number;
  currency: "UAH" | "USD";
  exchangeRate: number;
}

const COLORS = ["#10b981", "#3b82f6", "#8b5cf6", "#f59e0b", "#eab308", "#84cc16", "#22c55e", "#14b8a6", "#06b6d4", "#6366f1"];

export function IncomeChartDialog({
  open,
  onOpenChange,
  incomeBySource,
  totalIncome,
  currency,
  exchangeRate,
}: IncomeChartDialogProps) {
  const symbol = currency === "USD" ? "$" : "₴";
  
  const chartData = incomeBySource.map((item) => ({
    ...item,
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-slate-400">
            Загальна сума доходів: {symbol}{totalIncome > 0 ? (currency === "USD" ? (totalIncome / exchangeRate).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : totalIncome.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })) : "0.00"}
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="rounded-xl border border-slate-800/50 bg-gradient-to-br from-slate-900/80 to-slate-800/40 p-6">
                ...
                <ResponsiveContainer width="100%" height={350}>
                  <BarChart data={barChartData}>
                    ...
                    />
                    <Bar dataKey="Сума" fill="#10b981" radius={[8, 8, 0, 0]}>
