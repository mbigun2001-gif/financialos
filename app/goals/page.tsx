...
import { Target, Plus, ArrowLeft, Edit, X, TrendingUp, Calendar, Award, BarChart3, Sparkles, CheckCircle2, Trash2 } from "lucide-react";
...
export default function GoalsPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingGoal) {
      ...
      dataStore.goals.update(editingGoal.id, {
        ...
        incomeCategories: selectedIncomeCategories.length > 0 ? selectedIncomeCategories : undefined,
  ...
  const handleEdit = (goal: Goal) => {
    ...
    // Підтримка старого формату та нового
  ...
  // Дані для графіка прогресу по часу (останні 30 днів)
  const progressHistory = Array.from({ length: 30 }, (_, i) => {
    const date = new Date();
    date.setDate(date.getDate() - (29 - i));
    // Симулюємо історію (в реальності це має зберігатися)
  ...
  // Розрахунок прогнозу досягнення для кожної цілі
  const calculateForecast = (goal: Goal) => {
    ...
    // Розраховуємо середній прогрес за останній тиждень (якщо є транзакції)
    ...
    return {
      ...
      dailyNeeded,
      averageDaily,
      canAchieve: dailyNeeded <= averageDaily * 1.5, // Можна досягти якщо потрібно не більше ніж 1.5x від середнього
    };
  };
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex gap-2 bg-slate-800/50 p-1 rounded-lg">
            ...
            <Button
              ...
              className={viewMode === "analytics" ? "bg-blue-600" : ""}
            >
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card className="border-slate-800 bg-gradient-to-br from-blue-900/20 to-slate-900/50">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  placeholder="Наприклад: Досягти $10,000 доходу на місяць"
                />
              </div>
              <div>
                  ...
                  {selectedIncomeCategories.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {selectedIncomeCategories.map((cat) => (
                        ...
                        >
                          <span className="text-sm text-blue-300">{cat}</span>
                          <button
                            type="button"
              ...
              <div className="flex gap-3">
                ...
                <Button
                  type="submit"
                  className="flex-1 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 shadow-lg shadow-blue-500/20 hover:shadow-blue-500/30 transition-all duration-200"
                >
          ...
          {goals.map((goal) => {
            ...
            return (
              <motion.div
                ...
                transition={{ duration: 0.3 }}
              >
                <Card className={`border-slate-800 bg-gradient-to-br from-slate-900/80 to-slate-800/40 hover:border-slate-700 transition-all relative ${isCompleted ? 'ring-2 ring-green-500/50' : ''}`}>
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <CardTitle className="text-slate-100 flex items-center gap-2 flex-1">
                      <Target className={`h-5 w-5 ${isCompleted ? 'text-green-400' : 'text-blue-400'}`} />
                      {goal.title}
                    </CardTitle>
                    <div className="flex items-center gap-2">
                      {isCompleted && (
                        <div className="flex items-center gap-1 text-green-400">
                          ...
                          <span className="text-xs font-medium">Досягнуто!</span>
                        </div>
                      )}
                      <Button
                        variant="ghost"
                        ...
                        className="h-6 w-6 p-0 text-slate-400 hover:text-red-400 hover:bg-red-500/10 rounded-full"
                        title="Видалити ціль"
                      >
                        <X className="h-4 w-4" />
                      </Button>
